...
- The full-page rule must not create a cramped resume. If Experience or Projects becomes visually dense, redistribute whitespace first by relaxing negative spacing, tightening less important areas, shortening low-value bullets, or removing weaker projects. A slightly less packed bottom is better than a resume that looks hard to read.
...
- Prefer fewer, stronger projects with readable descriptions over many project one-liners stacked tightly. Projects should support the target role and remain scannable by a human reviewer.
